...
/* =========================================================
   TOP TICKER: July 4 Countdown (site-wide safe)
   Paste at BOTTOM of main.js
========================================================= */
...
(() => {
  ...
  const elDays = root.querySelector("[data-july-days]");
  const elHours = root.querySelector("[data-july-hours]");
  const elMins = root.querySelector("[data-july-mins]");
  const elSecs = root.querySelector("[data-july-secs]");
  ...
  const getTarget = () => {
    ...
    // July 4th at 12:00 AM local time
    let target = new Date(year, 6, 4, 0, 0, 0, 0);

    // If already past July 4th, target next year
    if (now.getTime() >= target.getTime()) {
      target = new Date(year + 1, 6, 4, 0, 0, 0, 0);
    }
    return target;
  };
  ...
  const update = () => {
    ...
    // If we hit July 4 exactly (or passed), re-target next year
    if (diff <= 0) {
      target = getTarget();
    }
    ...
    // Update + animate only when changed
    if (last.d !== days) { elDays.textContent = String(days); tickAnim(elDays); last.d = days; }
    if (last.h !== hours) { elHours.textContent = pad2(hours); tickAnim(elHours); last.h = hours; }
    if (last.m !== mins) { elMins.textContent = pad2(mins); tickAnim(elMins); last.m = mins; }
    if (last.s !== secs) { elSecs.textContent = pad2(secs); tickAnim(elSecs); last.s = secs; }
  };

  update();
  const timer = setInterval(update, 1000);

  // Clean up if page is hidden a long time (optional)
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) update();
  });

  // Just in case you ever want to stop it:
  root.dataset.julyTimer = String(timer);
})();
